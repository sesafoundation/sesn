...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of tSESAs to pay out per user request")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s tSESA", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte, crc20Tokens map[string]crc20Info, datadir string) (*faucet, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the SESN service: %w", err)
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No SESA Network address found to fund")
	}
	var avatar string
