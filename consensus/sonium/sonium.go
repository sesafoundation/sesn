...
var (
	...
	//BlockReward = big.NewInt(1e+18)  // Block reward 1

	//BlockReward = big.NewInt(9e+18)  // Block reward 9
...
// Sonium is the proof-of-staking consensus engine of SESN(SESA Network).
type Sonium struct {
	chainConfig *params.ChainConfig
	config      *params.SoniumConfig // Consensus engine configuration parameters
...
// New creates a Sonium consensus engine
func New(chainConfig *params.ChainConfig, db ethdb.Database, ethAPI *ethapi.PublicBlockChainAPI) *Sonium {
	soniumConfig := *chainConfig.Sonium
	if soniumConfig.Epoch == 0 {
		soniumConfig.Epoch = defaultEpochLength
	}
...
func (s *Sonium) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, s.signatures, s.chainConfig.ChainID)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (s *Sonium) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (s *Sonium) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (s *Sonium) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (s *Sonium) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (s *Sonium) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (s *Sonium) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (s *Sonium) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (s *Sonium) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (s *Sonium) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (s *Sonium) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, allLogs *[]*types.Log, receipts *[]*types.Receipt, systemTxs *[]*types.Transaction, usedGas *uint64) error {
	chainContext := chainContext{Chain: chain, sonium: s}
...
func (s *Sonium) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, []*types.Receipt, error) {
	chainContext := chainContext{Chain: chain, sonium: s}
	// Initialize all system contracts at block 1
	if header.Number.Cmp(common.Big1) == 0 {
		if err := s.initializeSystemContracts(chain, state, header, chainContext,
...
// with.
func (s *Sonium) Authorize(validator common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Sonium) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: validator}, accounts.MimetypeSonium, SoniumRLP(header, s.chainConfig.ChainID))
	if err != nil {
		return err
	}
...
func (s *Sonium) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (s *Sonium) SealHash(header *types.Header) common.Hash {
	return SealHash(header, s.chainConfig.ChainID)
}

// Close implements consensus.Engine. It's a noop for sonium as there are no background threads.
func (s *Sonium) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (s *Sonium) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "sonium",
		Version:   "1.0",
		Service:   &API{chain: chain, sonium: s},
		Public:    false,
	}}
}
...
// SoniumRLP returns the rlp bytes which needs to be signed for the proof-of-authority
// sealing. The RLP to sign consists of the entire header apart from the 65 byte signature
// contained at the end of the extra data.
// Note, the method requires the extra data to be at least 65 bytes, otherwise it
// panics. This is done to avoid accidentally using both forms (signature present
// or not), which could be abused to produce different hashes for the same header.
func SoniumRLP(header *types.Header, chainID *big.Int) []byte {
	b := new(bytes.Buffer)
...
func (s *Sonium) initializeSystemContracts(chain consensus.ChainHeaderReader, state *state.StateDB, header *types.Header, chainContext core.ChainContext,
...
func (s *Sonium) getTopValidator(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
...
func (s *Sonium) slash(chain consensus.ChainHeaderReader, state *state.StateDB, header *types.Header, chainContext core.ChainContext,
	txs *[]*types.Transaction, allLogs *[]*types.Log, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	number := header.Number.Uint64()
	snap, err := s.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
...
func (s *Sonium) distributeReward(chain consensus.ChainHeaderReader,
...
func (s *Sonium) collectBlockReward(chain consensus.ChainHeaderReader,
...
func (s *Sonium) epochProcess(chain consensus.ChainHeaderReader,
	state *state.StateDB, header *types.Header, chainContext core.ChainContext,
...
func (s *Sonium) updateActivatedValidators(chain consensus.ChainHeaderReader,
	state *state.StateDB, header *types.Header, chainContext core.ChainContext,
	txs *[]*types.Transaction, allLogs *[]*types.Log, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
...
func (s *Sonium) decreaseMissedBlocksCounter(chain consensus.ChainHeaderReader,
	state *state.StateDB, header *types.Header, chainContext core.ChainContext,
	txs *[]*types.Transaction, allLogs *[]*types.Log, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
...
func (s *Sonium) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	if tx.To() == nil {
		return false, nil
	}
...
func (s *Sonium) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (s *Sonium) wrapSystemContractMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From:     from,
