...
var (
	...
	ApplicationSonium = SigFormat{
		accounts.MimetypeSonium,
		0x03,
	}
	TextPlain = SigFormat{
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case ApplicationSonium.Mime:
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationSonium.Mime)
		}
		soniumData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(soniumData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming sonium header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
			// Need to add it back, to get a suitable length for hashing
			newExtra := make([]byte, len(header.Extra)+65)
			copy(newExtra, header.Extra)
		...
		// Get back the rlp data, encoded by us
		sighash, soniumRlp, err := soniumHeaderHashAndRlp(header, api.chainID)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*NameValueType{
			{
				Name:  "Sonium header",
				Typ:   "sonium",
				Value: fmt.Sprintf("sonium header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Sonium uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: soniumRlp, Messages: messages, Hash: sighash}
...
func soniumHeaderHashAndRlp(header *types.Header, chainId *big.Int) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("sonium header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = sonium.SoniumRLP(header, chainId)
	hash = sonium.SealHash(header, chainId).Bytes()
	return hash, rlp, err
}
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// https://github.com/sesafoundation/sesn/wiki/Management-APIs#personal_ecRecover
	if len(sig) != 65 {
