...
func InspectDatabase(db ethdb.Database) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("sonium-")) && len(key) == 8+common.HashLength:
			soniumSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Sonium snapshots", soniumSnaps.Size(), soniumSnaps.Count()},
