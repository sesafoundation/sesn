...
import (
	...
	"github.com/sesafoundation/sesn/crypto/blake2b"
	"github.com/sesafoundation/sesn/crypto/bls12381"
	"github.com/sesafoundation/sesn/crypto/bn256"
...
func RunPrecompiledContract(p PrecompiledContract, input []byte, suppliedGas uint64, contract *Contract, evm *EVM) (ret []byte, remainingGas uint64, err error) {
    gasCost := p.RequiredGas(input)
    if suppliedGas < gasCost {
        return nil, 0, ErrOutOfGas
    }
    suppliedGas -= gasCost
    output, err := p.Run(input, contract, evm)
